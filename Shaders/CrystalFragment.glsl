...
void main(void) {
	...
	vec4 yDiff = texture(diffuseTex, yUV);
	vec4 xDiff = texture(diffuseTex, xUV);
	vec4 zDiff = texture(diffuseTex, zUV);

	vec3 blendWeights = abs(IN.normal);
	blendWeights = blendWeights / (blendWeights.x + blendWeights.y + blendWeights.z);

	vec4 diffuse = xDiff * blendWeights.x + yDiff * blendWeights.y + zDiff * blendWeights.z;

	yDiff = texture(bumpTex, yUV);
	xDiff = texture(bumpTex, xUV);
	zDiff = texture(bumpTex, zUV);

	vec4 bump = xDiff * blendWeights.x + yDiff * blendWeights.y + zDiff * blendWeights.z;
	
	mat3 TBN = mat3(normalize(IN.tangent), normalize(IN.binormal), normalize(IN.normal));	
	vec3 bumpNormal = normalize(TBN * normalize(bump.rgb * 2.0 - 1.0)); 
	
	vec3 reflectDir = reflect(-viewDir, normalize(IN.normal));
	vec4 reflectTex = texture(cubeTex, reflectDir);
	vec4 reflectCol = reflectTex + (diffuse * 0.25f) + IN.colour;
	...
	fragColour.a = 1;
	fragColour.a *= clamp(viewDistance / (heightmapDim.x * 0.01875f), 0.75, 1.0);
}